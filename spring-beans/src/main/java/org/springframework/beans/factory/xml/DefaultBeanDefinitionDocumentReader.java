...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	public void registerBeanDefinitions(Document doc, XmlReaderContext readerContext) {
		...
		//获取XML Document Root Element并执行注册
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		// 保存原有的 BeanDefinitionParserDelegate 对象
		BeanDefinitionParserDelegate parent = this.delegate;
		// <1> 创建 BeanDefinitionParserDelegate 对象，并进行设置到 delegate
		this.delegate = createDelegate(getReaderContext(), root, parent);

		// <2> 给定的ＵＲＬ是否包含默认命名空间检查 <beans /> 根标签的命名空间是否为空，或者是 http://www.springframework.org/schema/beans
		...
		// 设置 delegate 为开头保存的　BeanDefinitionParserDelegate 对象
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		// 进行 bean 元素解析。
		//　BeanDefinitionParserDelegate负责解析`BeanDifinition`对象，定义了解析XML Element的各种方法
